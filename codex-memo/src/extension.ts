...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider("codexMemo.memos", provider, {
...
class MemoStore implements vscode.Disposable {
  private data: MemoFile = { version: 1, memos: [] };
  ...
  getMemoFileUri(): vscode.Uri | undefined {
  ...
  getMemosForDocument(document: vscode.TextDocument): Memo[] {
  ...
  toWorkspacePath(uri: vscode.Uri): string | null {
  ...
  async openMemoDocument(memo: Memo): Promise<vscode.TextDocument | null> {
    if (!this.rootUri) {
      return null;
    }
    try {
  ...
  watch(onChange: () => Promise<void>): vscode.Disposable {
    if (!this.rootUri) {
      return new vscode.Disposable(() => undefined);
    }

    const watcher = vscode.workspace.createFileSystemWatcher(
      new vscode.RelativePattern(this.rootUri, MEMO_FILE_NAME)
    );
    const run = debounce(() => {
      void onChange();
    }, 100);
  ...
  private async resolveMemoRootUri(): Promise<vscode.Uri | undefined> {
    const active = vscode.window.activeTextEditor;
    if (active) {
...
class MemoViewProvider implements vscode.WebviewViewProvider {
  ...
  private async editMemo(id: string, body: string): Promise<void> {
    if (!body.trim()) {
      return;
    }
  ...
  private async addReply(id: string, body: string): Promise<void> {
    if (!body.trim()) {
      return;
    }
  ...
  private async editReply(id: string, replyId: string, body: string): Promise<void> {
    if (!body.trim()) {
      return;
    }
    const memo = this.store.getAll().find((candidate) => candidate.id === id);
  ...
  private async deleteReply(id: string, replyId: string): Promise<void> {
    const memo = this.store.getAll().find((candidate) => candidate.id === id);
    if (!memo) {
      return;
    }
...
class DecorationManager implements vscode.Disposable {
  ...
  apply(editor: vscode.TextEditor, memos: Memo[]): void {
...
function getSelectionOrWord(editor: vscode.TextEditor): vscode.Range {
...
function rangeToAnchor(range: vscode.Range): Anchor {
  return {
...
function anchorToRange(anchor: Anchor): vscode.Range {
  return new vscode.Range(
    new vscode.Position(anchor.startLine, anchor.startCharacter),
    new vscode.Position(anchor.endLine, anchor.endCharacter)
  );
}
...
function compareMemoFileAndPosition(a: Memo, b: Memo): number {
  return a.file.localeCompare(b.file) || compareMemoPosition(a, b);
}

function getVisibleWindow(editor: vscode.TextEditor): VisibleWindow | null {
  if (!editor.visibleRanges.length) {
    return null;
  }

  return editor.visibleRanges.reduce<VisibleWindow>((window, range) => ({
...
function getAlignmentSettings(): AlignmentSettings {
  const config = vscode.workspace.getConfiguration("codexMemo.alignment");
  return {
    topOffsetPx: config.get<number>("topOffsetPx", 0),
    lineHeightPx: config.get<number>("lineHeightPx", 0),
    lineScale: config.get<number>("lineScale", 1),
    cardAnchorOffsetPx: config.get<number>("cardAnchorOffsetPx", 0)
  };
}
...
async function scanForGitRoot(root: string, maxDepth: number): Promise<string | null> {
  try {
